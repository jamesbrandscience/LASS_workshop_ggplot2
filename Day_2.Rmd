...
# Before we start doing practical things...

We will continue using the `tidyverse` to explore and visualise data, to do so we need to do the following:

1. Tell `R` that we want to use the `tidyverse` package... If this loads with no problems, you do not have to do anything else just now

```{r}
library(tidyverse)
```

2. If you were not able to install and load the `tidyverse`, we can get around this by running this code...
...
3. If you have any problems installing or loading these... put up your hand.
...
The `ggworldcloud` package has a nice dataset we to show how to use __wordclouds__ in `ggplot` - it is called `love_words_small`.
...
`lang` - The language the word is written in

`word` - The word for _love_ in the language

`native_speakers` - The number of native speakers (in millions)

`speakers` - The total number of speakers (in millions)

Let's visualise this dataset to see which __word__ has the most __speakers__

Note: In our `aes` argument, we need to use `label = word` and `size = speakers`, not the familiar `x = x` and `y = y` as we are not doing a typical `x~y` plot.

```{r}
ggplot(data = love_words_small[unique(love_words_small$word), ], aes(label = word, size = speakers, color = "red", family = 'Arial Unicode MS')) +
  geom_text_wordcloud() +

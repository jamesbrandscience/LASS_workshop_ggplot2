...
# Introduction to Day 2 {.unnumbered}
...
![Source: http://phdcomics.com/comics.php?f=1796](Images/Pie.jpg)
...
Note that there are 11 rows in this dataset, you can see the last row by clicking on the _next_ button at the bottom.
...
Suppose that we are interested in exploring this dataset, specifically we want to know if any of the variable might be related in some way to each other, for example the _x1_ variable with the _y1_ variable.
...
Now let us get some information about the spread of the data, we can do this by adding additional `funs` or _functions_ to our `summarise_all` argument. Here we will change the `summarise_all(mean)` part of the code, so that it reads `summarise_all(funs(mean, sd))`

<div class = "blue">
__Q.__ What will this add to our output?
</div>
...
Is there anything interesting you notice about these values from __x2__ and __y2__?
...
__Q.__ Now look at the raw data and plot them again
</div>
...
However this does not mean that we can let the computer do all the work for us, producing an informative, interpretable and accurate visualisation can sometimes be challenging, requiring you to make certain choices, but getting it right can really impact on how somebody reads and understand your work.
...
## Your first plot
First let us load in the `tidyverse` library, where `ggplot2` is stored.

```{r}
library(tidyverse)
```

Next, we can start by running the most simple plot using `ggplot2`...
...
* Finally, a layer of salient features
...
The types of variables have __levels__ which are independent from each other, e.g. possible levels for a geographical region variable would be, _Christchurch_, _Wellington_, _Auckland_, _Invercargill_ etc.
